...
public class PNG2JPEG implements ActionListener{
    ...
    PNG2JPEG() {
        ...
        ImageIcon png = new ImageIcon("pngpic.png");
           ...
           if (e.getSource() == selectFileButton) {
                ...
                if (file != null) {
                    ...
                    if (response == JFileChooser.APPROVE_OPTION) {
                        ...
                        String outputFilePath = new File(destinationFolder, file.getName().replace(".png", ".jpg")).getAbsolutePath();
                        pngToJpeg(file, outputFilePath);
                    }
                } else {
                    JOptionPane.showMessageDialog(null, "Please select a PNG file first.");
                }
            }
        }
        public static void pngToJpeg(File inputFile, String outputFilePath) {
            try {
                BufferedImage image = ImageIO.read(inputFile);
                BufferedImage rgbImage = new BufferedImage(image.getWidth(), image.getHeight(), BufferedImage.TYPE_INT_RGB);
                rgbImage.createGraphics().drawImage(image, 0, 0, null);
    
                ImageIO.write(rgbImage, "jpg", new File(outputFilePath));
                JOptionPane.showMessageDialog(null, "JPEG image downloaded successfully.");
            } catch (IOException e) {
                JOptionPane.showMessageDialog(null, "Error converting PNG to JPEG: " + e.getMessage());
            }
        }
}
