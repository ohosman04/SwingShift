...
public class MP3toWAV implements ActionListener{
    ...
    MP3toWAV() {
        ...
        ImageIcon mp3 = new ImageIcon("wav.png");
        ImageIcon wav = new ImageIcon("mp3.png");
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       if (e.getSource() == selectFileButton) {
        ...
        if (selectedFile != null && selectedFile.exists()) {
            JFileChooser fileChooser = new JFileChooser();
            fileChooser.setSelectedFile(new File(selectedFile.getName().replace(".mp3", ".wav")));
            int response = fileChooser.showSaveDialog(null);
            if (response == JFileChooser.APPROVE_OPTION) {
                File destinationFile = fileChooser.getSelectedFile();
                String outputWavPath = destinationFile.getAbsolutePath();
                convertMp3ToWav(selectedFile.getAbsolutePath(), outputWavPath);
            }
        } else {
            JOptionPane.showMessageDialog(null, "Please select an MP3 file to convert.");
        }
    }
    }

    public void convertMp3ToWav(String inputMp3File, String outputWavFile) {
        try {
            String lamePath = "\"C:\\Lame\\lame.exe\"";
    
            String lameCommand = lamePath + " --silent --decode \"" + inputMp3File + "\" \"" + outputWavFile + "\"";
            ...
            if (exitCode != 0) {
                JOptionPane.showMessageDialog(null, "Failed to convert MP3 to WAV. Exit code: " + exitCode);
            } else {
                JOptionPane.showMessageDialog(null, "MP3 to WAV conversion successful!");
            }
        } catch (IOException | InterruptedException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Failed to convert MP3 to WAV.");
        }
    }
}
